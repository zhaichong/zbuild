...
def cmd_create_order_dir(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    form = config.get("form", {}) if isinstance(config.get("form"), dict) else {}
    ...
    order_notes = payload.get("order_notes") or payload.get("orderNotes") or config.get("order_notes") or form.get("orderNotes") or form.get("order_notes") or ""
